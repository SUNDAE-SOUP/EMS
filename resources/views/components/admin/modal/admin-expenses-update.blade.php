    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mainContent">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li>
                            <div class="flex items-center">
                              ...
                              <a href="#" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">Expenses</a>
                            </div>
                        </li>
                    </ol>
                </div>
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $expenseList->expense_category->expense_category_name }}
                                </th>
                                <td class="px-6 py-4">
                                    {{ $expenseList->amount }}
                                </td>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div class="flex items-center justify-center rounded h-28 dark:bg-gray-800">
                    <button type="button" class="text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Add Expense</button>
                </div>
                ...
                <div id="roleUpdateModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative w-full max-w-md max-h-full">
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" id="modalContent">
                            <a href="{{route('admin.expensesTab')}}">
                            ...
                            <div class="px-6 py-6 lg:px-8" id="modalPlate">
                                <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Update Expense</h3>
                                <form method="POST" class="space-y-6" action="{{route('expense.update', $expense->id)}}">
                                    @csrf
                                    @method('PUT')
                                        ...
                                        <select name="expense_category_id" id="expense_category_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
                                            ...
                                            <option value="{{ $expenseCategory->id }}" @if ($expense->expense_category_id == $expenseCategory->id) selected @endif>
                                                {{ $expenseCategory->expense_category_name }}
                                            </option>
                                            @endforeach
                                        </select>
                                    <div>
                                        <label for="amount" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Description</label>
                                        <input type="number" value="{{$expense->amount}}" name="amount" id="amount" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                                    </div>
                                    <div>
                                        <label for="entry_date" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Description</label>
                                        <input type="date" value="{{$expense->entry_date}}" name="entry_date" id="entry_date" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                                    </div>
                                ...
                                <form method="POST" action="{{route('expense.softDelete', $expense->id)}}">
                                    @csrf
                                    @method('PUT')
